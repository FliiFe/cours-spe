...
\begin{dfn}[Hors-Programme]
    Un anneau $A$ commutatif est dit euclidien\index{anneau!euclidien} lorsqu'il existe une application $v:A\setminus\{0\} \to \N$ qui satisfait les propriétés suivantes:
    \begin{enumerate}
        \item \[
...
\begin{defprop}
    ...
    \begin{concenum}
    \item $\varphi:\Z\longrightarrow G, n\longmapsto a^n$ est un morphisme. Il existe un unique entier naturel non nul tel que $\Ker \varphi=d\Z$, qu'on appelle \textbf{ordre} de $a$ dans $G$\index{groupe!ordre}, noté $\ord_G (a)$ ou $\ord (a)$
